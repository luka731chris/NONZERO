...
async function writePayload(env,storageKey,state,opts={}){
 ...
 if(currentRev>0 && opts.baseRevision==null){
  const err=new Error('revision_required');err.code='revision_required';err.current=current;throw err;
 }
...
export default {
 async fetch(request,env){
  ...
  if(url.pathname==='/health')return json({ok:true,service:'nonzero-sync',version:'4.7',ergDataFirst:true,quoteDiscovery:true,rokuPairing:true,durablePairing:true,pairingModel:'stateless-v2',legacyPairing:true,revisionedState:true,canonicalState:true,bikeErgAuto:true,simpleShortcut:true,directShortcut:true,wallHealth:true,flightRecorder:true,revisionRequired:true,concept2:!!env.C2_API_TOKEN});
  ...
  if(url.pathname==='/wall/heartbeat'&&request.method==='POST'&&request.headers.get('X-NONZERO-Wall')){
   try{
    const token=String(request.headers.get('X-NONZERO-Wall')||'').trim();
    let storageKey=await decodeDurableWallToken(token);
    if(!storageKey)storageKey=await env.NONZERO_STATE.get('wall:'+await hashKey(token));
    if(!storageKey)return json({error:'missing_or_invalid_key'},401);
    let body={};try{body=await request.json()}catch{}
    const health={at:Date.now(),mode:String(body.mode||'unknown'),uptimeSeconds:Number(body.uptimeSeconds||0),activeSessionId:body.activeSessionId||null,lastStateAt:Number(body.lastStateAt||0),version:String(body.version||'unknown')};
    await env.NONZERO_STATE.put('wallhealth:'+storageKey,JSON.stringify(health),{expirationTtl:172800});
    return json({ok:true});
   }catch(e){return json({error:e.message||'wall_heartbeat_failed'},401)}
  }
  ...
  if(url.pathname==='/wall/health'&&request.method==='GET'){
   const raw=await env.NONZERO_STATE.get('wallhealth:'+storageKey);
   return json({ok:true,health:raw?JSON.parse(raw):null});
  }
  ...
  if(url.pathname==='/state'){
   ...
   if(request.method==='PUT'){
    ...
    try{
     ...
     if(e.code==='revision_required')return json({error:'revision_required',current:e.current},428);
